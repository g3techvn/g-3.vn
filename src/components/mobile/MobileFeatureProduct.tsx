...
const MobileFeatureProduct: React.FC<MobileFeatureProductProps> = ({ 
  products, 
  brands,
  loading, 
  error 
  ...
  // Biến đổi mảng brands thành map để dễ truy cập
  ...
  const productsByBrand = products && products.length > 0 
  ...
  if (Object.keys(productsByBrand).length > 0) {
    Object.keys(productsByBrand).forEach(brandId => {
      productsByBrand[brandId].sort((a, b) => a.price - b.price);
    });
  }

  const handleScroll = (brandId: string) => {
